...
var LOG_QUERIES = true; //It shows all executed queries
var LOG_DEBUG = true;  //It show messages which allow you to follow the code
...
module.exports = (function () {
    ...
    var adapter = {
        ...
        define: function (connection, collection, definition, cb) {
            ...
            var self = this;
                ...
                var collectionN = connectionObject.collections[collection];
                if (!collectionN) {
                    return cb(util.format('Unknown collection `%s` in connection `%s`', collectionName, connectionName));
                }
                ...
                // Run query
                execQuery(connections[connection],query, [], function __DEFINE__(err, result) {
                    if (err) {
                    ...
                    self.describe(connection, collection, function(err) {
                        cb(err, result);
                    });
                });
        },
        ...
        // Add a new row to the table - PENDIENTE
        ...
        update: function(connectionName, table, options, data, cb) {
            ...
            for(var k=0; k <keysoptions.length; k++){
                options.where[ "where"+keysoptions[k] ] = options.where[ keysoptions[k] ];
                data["where"+keysoptions[k]] = options.where[ keysoptions[k] ];
                delete options.where[ keysoptions[k] ];
            }
            var keysoptions = Object.keys(options.where);
            for (var z = keys.length; z < keys.length + keysoptions.length + 1; z++) {
                query.query = query.query.replace(":" + z, ":" + keysoptions[z - 1 - keys.length]);
            }
            sails.log.warn(options.where);
            sails.log.info(data);
            //sails.log.info(options);
            
            execQuery(connections[connectionName], query.query, data, function (err, results) {
                if (err) {
                    return cb(handleQueryError(err));
                }
                //ToDo return afected registries
                cb();
            });
    ...
    function execQuery(connection,query,data,cb){
        ...
        pool.getConnection(function (err, connection) {
            connection.execute(query, data, {autoCommit: true}, function (err, result) {
                doRelease(connection);
                cb(err, result);
            });
        });
